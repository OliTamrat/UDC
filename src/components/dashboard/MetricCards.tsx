...
function StationsModal({ metrics, isDark, router }: { metrics: Metrics; isDark: boolean; router: ReturnType<typeof useRouter> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className={`rounded-xl border p-3 text-center ${s.bg}`}>
            ...
            <div className={`text-xs ${isDark ? "text-[#E5E7EB]" : "text-[#4B5563]"}`}>{s.label}</div>
          </div>
        ))}
      </div>

      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>By Type</h4>
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={t.label} className={`flex items-center gap-3 p-2.5 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F9FAFB]"}`}>
              <div className={`text-sm font-semibold ${t.color}`}>{t.count}</div>
              <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{t.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>All Stations</h4>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{s.id} &middot; {s.type.replace("-", " ")}</div>
                </div>
              </div>
              <ExternalLink className={`w-3 h-3 ${isDark ? "text-[#9CA3AF]" : "text-[#9CA3AF]"}`} />
            </button>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-xl border p-4 ${isDark ? "bg-green-500/10 border-green-500/20" : "bg-green-50 border-green-200"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>stations &ge; 5.0 mg/L</div>
        </div>
        <div className={`rounded-xl border p-4 ${isDark ? "bg-red-500/10 border-red-500/20" : "bg-red-50 border-red-200"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>stations &lt; 5.0 mg/L</div>
        </div>
      </div>

      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>Current Readings by Station</h4>
      ...
      <div className={`rounded-xl p-4 ${isDark ? "bg-blue-500/5 border border-blue-500/20" : "bg-blue-50 border border-blue-200"}`}>
        <div className="flex items-start gap-2">
          ...
          <div className={`text-xs leading-relaxed ${isDark ? "text-[#E5E7EB]" : "text-[#4B5563]"}`}>
...
function TempModal({ metrics, isDark }: { metrics: Metrics; isDark: boolean }) {
  return (
    <div className="space-y-5">
      ...
      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>Current Readings</h4>
        <div className="space-y-1">
          {metrics.active.map((s) => {
            const temp = s.lastReading?.temperature || 0;
            return (
              <div key={s.id} className={`flex items-center justify-between p-2 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F9FAFB]"}`}>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={r.label} className={`rounded-xl border p-3 text-center ${r.bg}`}>
            ...
            <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{r.range}</div>
          </div>
        ))}
      </div>
      ...
      <div className={`rounded-xl border p-4 ${isDark ? "bg-emerald-500/10 border-emerald-500/20" : "bg-emerald-50 border-emerald-200"}`}>
        <div className="text-center">
          ...
          <div className={`text-xs mt-1 ${isDark ? "text-[#E5E7EB]" : "text-[#4B5563]"}`}>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-xl border p-4 ${isDark ? "bg-amber-500/10 border-amber-500/20" : "bg-amber-50 border-amber-200"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>
      ...
      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>Interpretation Guide</h4>
        <div className="space-y-1.5">
          {[
            ...
            <div key={r.range} className={`flex items-center gap-3 p-2 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F9FAFB]"}`}>
              ...
              <div>
                ...
                <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{r.desc}</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-5">
      ...
      {metrics.ecoliAlerts.length > 0 ? (
          ...
          <div className="space-y-2">
            {metrics.ecoliAlerts.map((s) => (
              ...
              >
                <div>
                  <div className={`text-xs font-medium ${isDark ? "text-white" : "text-[#111827]"}`}>{s.name}</div>
                  <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{s.id}</div>
                </div>
        ...
        <div className={`rounded-xl border p-4 text-center ${isDark ? "bg-green-500/10 border-green-500/20" : "bg-green-50 border-green-200"}`}>
          ...
          <div className={`text-xs mt-1 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>No active E. coli alerts</div>
        </div>
      )}
        ...
        {metrics.giStations.map((s) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className={`text-[10px] mt-0.5 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{s.id} &middot; {s.parameters?.join(", ")}</div>
            </div>
            <ExternalLink className={`w-3 h-3 mt-1 flex-shrink-0 ${isDark ? "text-[#9CA3AF]" : "text-[#9CA3AF]"}`} />
          </button>
        ))}
      </div>
          ...
          <div key={p.title} className={`p-3 rounded-xl border ${isDark ? "border-white/[0.06] bg-white/5" : "border-[#E5E7EB] bg-[#F9FAFB]"}`}>
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className={`text-[10px] mt-0.5 ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>PI: {p.pi} &middot; {p.dept}</div>
              </div>
            ...
            >
              <div className="flex items-start justify-between mb-1 sm:mb-2">
                ...
                <div className="flex items-center gap-1">
                  {card.getTotal && (
                    <span className={`text-[10px] sm:text-xs ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>/ {card.getTotal(metrics)}</span>
                  )}
                  <Info className={`w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`} />
                </div>
              </div>
              <div className="flex items-baseline gap-1">
                <span className={`text-lg sm:text-2xl font-bold ${card.color}`}>{card.getValue(metrics)}</span>
                {card.unit && <span className={`text-xs ${isDark ? "text-[#D1D5DB]" : "text-[#4B5563]"}`}>{card.unit}</span>}
              </div>
              <p className={`text-xs mt-1 ${isDark ? "text-[#E5E7EB]" : "text-[#4B5563]"}`}>{card.label}</p>
