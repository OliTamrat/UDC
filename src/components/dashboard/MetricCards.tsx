...
const cardConfigs: MetricCardConfig[] = [
  {
    ...
    lightBorderColor: "border-green-300",
  ...
  },
  {
    ...
    lightBorderColor: "border-emerald-300",
  ...
  },
  {
    ...
    lightBorderColor: "border-red-300",
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Active", count: byStatus.active.length, color: "text-green-400", bg: isDark ? "bg-green-500/10 border-green-500/20" : "bg-green-100 border-green-300" },
          { label: "Maintenance", count: byStatus.maintenance.length, color: "text-amber-400", bg: isDark ? "bg-amber-500/10 border-amber-500/20" : "bg-amber-100 border-amber-300" },
          { label: "Offline", count: byStatus.offline.length, color: "text-[#D1D5DB]", bg: isDark ? "bg-[#6B7280]/10 border-[#6B7280]/20" : "bg-[#F0F1F3] border-[#D1D5DB]" },
        ].map((s) => (
          <div key={s.label} className={`rounded-xl border p-3 text-center ${s.bg}`}>
            <div className={`text-2xl font-bold ${s.color}`}>{s.count}</div>
            <div className={`text-xs ${isDark ? "text-[#E5E7EB]" : "text-[#1F2937]"}`}>{s.label}</div>
          </div>
        ))}
      </div>

      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>By Type</h4>
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={t.label} className={`flex items-center gap-3 p-2.5 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F0F1F3]"}`}>
              <div className={`text-sm font-semibold ${t.color}`}>{t.count}</div>
              <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{t.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>All Stations</h4>
          ...
          {metrics.stations.map((s) => (
            <button
              ...
              className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-colors ${
                isDark ? "hover:bg-white/5" : "hover:bg-[#E5E7EB]"
              }`}
            >
              <div className="flex items-center gap-2">
                <MapPin className={`w-3.5 h-3.5 ${s.status === "active" ? "text-green-400" : s.status === "maintenance" ? "text-amber-400" : "text-[#D1D5DB]"}`} />
                <div>
                  <div className={`text-xs font-medium ${isDark ? "text-white" : "text-[#111827]"}`}>{s.name}</div>
                  <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{s.id} &middot; {s.type.replace("-", " ")}</div>
                </div>
              </div>
              <ExternalLink className={`w-3 h-3 ${isDark ? "text-[#6B7280]" : "text-[#6B7280]"}`} />
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-xl border p-4 ${isDark ? "bg-green-500/10 border-green-500/20" : "bg-green-100 border-green-300"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>stations &ge; 5.0 mg/L</div>
        </div>
        <div className={`rounded-xl border p-4 ${isDark ? "bg-red-500/10 border-red-500/20" : "bg-red-100 border-red-300"}`}>
          <div className="flex items-center gap-2 mb-1">
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>stations &lt; 5.0 mg/L</div>
        </div>
      </div>

      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>Current Readings by Station</h4>
        <div className="space-y-1">
          {metrics.active.map((s) => {
            ...
            return (
              <div key={s.id} className={`flex items-center gap-3 p-2 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F0F1F3]"}`}>
      ...
      <div className={`rounded-xl p-4 ${isDark ? "bg-blue-500/5 border border-blue-500/20" : "bg-blue-100 border border-blue-300"}`}>
        <div className="flex items-start gap-2">
          <Info className="w-4 h-4 text-blue-400 mt-0.5 flex-shrink-0" />
          <div className={`text-xs leading-relaxed ${isDark ? "text-[#E5E7EB]" : "text-[#1F2937]"}`}>
...
function TempModal({ metrics, isDark }: { metrics: Metrics; isDark: boolean }) {
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-xl border p-4 ${isDark ? "bg-cyan-500/10 border-cyan-500/20" : "bg-cyan-100 border-cyan-200"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{(metrics.avgTemp * 9 / 5 + 32).toFixed(1)}°F</div>
        </div>
        <div className={`rounded-xl border p-4 ${isDark ? "bg-white/5 border-white/[0.06]" : "bg-[#F0F1F3] border-[#D1D5DB]"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>89.6°F max for aquatic life</div>
        </div>
      </div>

      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>Current Readings</h4>
        <div className="space-y-1">
          {metrics.active.map((s) => {
            const temp = s.lastReading?.temperature || 0;
            return (
              <div key={s.id} className={`flex items-center justify-between p-2 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F0F1F3]"}`}>
                <span className={`text-xs ${isDark ? "text-[#E5E7EB]" : "text-[#374151]"}`}>{s.name.split(" ").slice(0, 3).join(" ")}</span>
      ...
      <div className={`rounded-xl p-4 ${isDark ? "bg-cyan-500/5 border border-cyan-500/20" : "bg-cyan-100 border border-cyan-200"}`}>
        <div className="flex items-start gap-2">
...
function PHModal({ metrics, isDark }: { metrics: Metrics; isDark: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Acidic", range: "0 — 6.4", color: "text-red-400", bg: isDark ? "bg-red-500/10 border-red-500/20" : "bg-red-100 border-red-300" },
          { label: "Optimal", range: "6.5 — 9.0", color: "text-green-400", bg: isDark ? "bg-green-500/10 border-green-500/20" : "bg-green-100 border-green-300" },
          { label: "Alkaline", range: "9.1 — 14", color: "text-purple-400", bg: isDark ? "bg-purple-500/10 border-purple-500/20" : "bg-purple-100 border-purple-300" },
        ].map((r) => (
          <div key={r.label} className={`rounded-xl border p-3 text-center ${r.bg}`}>
            <div className={`text-xs font-bold ${r.color}`}>{r.label}</div>
            <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{r.range}</div>
          </div>
        ))}
      </div>

      <div className={`rounded-xl border p-4 ${isDark ? "bg-emerald-500/10 border-emerald-500/20" : "bg-emerald-100 border-emerald-300"}`}>
        <div className="text-center">
          <div className="text-3xl font-bold text-emerald-400">{metrics.avgPH}</div>
          <div className={`text-xs mt-1 ${isDark ? "text-[#E5E7EB]" : "text-[#1F2937]"}`}>
            Network Average &middot; {inRange.length}/{metrics.active.length} stations in optimal range
          </div>
        </div>
      </div>

      <div className={`rounded-xl p-4 ${isDark ? "bg-emerald-500/5 border border-emerald-500/20" : "bg-emerald-100 border border-emerald-300"}`}>
        <div className="flex items-start gap-2">
...
function TurbidityModal({ metrics, isDark }: { metrics: Metrics; isDark: boolean }) {
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-xl border p-4 ${isDark ? "bg-amber-500/10 border-amber-500/20" : "bg-amber-100 border-amber-300"}`}>
          ...
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>
        ...
        <div className={`rounded-xl border p-4 ${isDark ? "bg-white/5 border-white/[0.06]" : "bg-[#F0F1F3] border-[#D1D5DB]"}`}>
          ...
          <div className="text-2xl font-bold text-[#6B7280]">&le;50 NTU</div>
          <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>Typical healthy freshwater</div>
        </div>
      </div>

      <div>
        <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>Interpretation Guide</h4>
        <div className="space-y-1.5">
          {[
            ...
            <div key={r.range} className={`flex items-center gap-3 p-2 rounded-lg ${isDark ? "bg-white/5" : "bg-[#F0F1F3]"}`}>
              <span className={`text-xs font-mono w-20 ${r.color}`}>{r.range}</span>
              <div>
                <div className={`text-xs font-medium ${isDark ? "text-white" : "text-[#111827]"}`}>{r.label}</div>
                <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{r.desc}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {metrics.ecoliAlerts.length > 0 ? (
          ...
          <div className="space-y-2">
            {metrics.ecoliAlerts.map((s) => (
              <button
                ...
                onClick={() => router.push(`/station/${s.id}`)}
                className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-colors ${
                  isDark ? "bg-red-500/5 hover:bg-red-500/10" : "bg-red-100 hover:bg-red-100"
                }`}
              >
                <div>
                  <div className={`text-xs font-medium ${isDark ? "text-white" : "text-[#111827]"}`}>{s.name}</div>
                  <div className={`text-[10px] ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{s.id}</div>
                </div>
        ...
        <div className={`rounded-xl border p-4 text-center ${isDark ? "bg-green-500/10 border-green-500/20" : "bg-green-100 border-green-300"}`}>
          <CheckCircle2 className="w-8 h-8 text-green-400 mx-auto mb-2" />
          <div className={`text-sm font-semibold ${isDark ? "text-green-400" : "text-green-700"}`}>All stations within EPA limits</div>
          <div className={`text-xs mt-1 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>No active E. coli alerts</div>
        </div>
      )}

      <div className={`rounded-xl p-4 ${isDark ? "bg-red-500/5 border border-red-500/20" : "bg-red-100 border border-red-300"}`}>
        <div className="flex items-start gap-2">
        ...
        {metrics.giStations.map((s) => (
          <button
            ...
            className={`w-full flex items-start gap-3 p-3 rounded-xl text-left transition-colors border ${
              isDark ? "border-green-500/20 bg-green-500/5 hover:bg-green-500/10" : "border-green-300 bg-green-100 hover:bg-green-100"
            }`}
          >
            <TreePine className="w-5 h-5 text-green-400 mt-0.5 flex-shrink-0" />
            <div className="flex-1">
              <div className={`text-sm font-medium ${isDark ? "text-white" : "text-[#111827]"}`}>{s.name}</div>
              <div className={`text-[10px] mt-0.5 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>{s.id} &middot; {s.parameters?.join(", ")}</div>
            </div>
            <ExternalLink className={`w-3 h-3 mt-1 flex-shrink-0 ${isDark ? "text-[#6B7280]" : "text-[#6B7280]"}`} />
          </button>
        ))}
      </div>

      <div className={`rounded-xl p-4 ${isDark ? "bg-green-500/5 border border-green-500/20" : "bg-green-100 border border-green-300"}`}>
        <div className="flex items-start gap-2">
...
function ResearchModal({ isDark, router }: { isDark: boolean; router: ReturnType<typeof useRouter> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {projects.map((p) => (
          <div key={p.title} className={`p-3 rounded-xl border ${isDark ? "border-white/[0.06] bg-white/5" : "border-[#D1D5DB] bg-[#F0F1F3]"}`}>
            <div className="flex items-start justify-between">
              <div>
                <div className={`text-xs font-semibold ${isDark ? "text-white" : "text-[#111827]"}`}>{p.title}</div>
                <div className={`text-[10px] mt-0.5 ${isDark ? "text-[#D1D5DB]" : "text-[#1F2937]"}`}>PI: {p.pi} &middot; {p.dept}</div>
              </div>
              <span className={`text-[10px] px-2 py-0.5 rounded-full border ${
                p.status === "Active"
                  ? isDark ? "text-green-400 bg-green-500/10 border-green-500/20" : "text-green-700 bg-green-100 border-green-300"
                  : isDark ? "text-amber-400 bg-amber-500/10 border-amber-500/20" : "text-amber-700 bg-amber-100 border-amber-300"
              }`}>{p.status}</span>
            </div>
          </div>
        ))}
